package org.ohdj.nfcaimereader.ui.component.dialog
...
import org.ohdj.nfcaimereader.ui.component.material.ExpressiveDialog
    ...
    if (showDialog.value) {
        ExpressiveDialog(
            ...
            text = visuals.content?.let { content ->
                {
                    when {
                        visuals.isMarkdown -> Text(text = content) // Simplified: markdown rendered as plain text (no network/webkit deps)
                        visuals.isHtml -> Text(text = content) // Simplified: html rendered as plain text
